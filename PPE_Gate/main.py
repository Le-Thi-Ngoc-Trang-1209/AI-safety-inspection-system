...
ok = cv2.imread("icon/ok.png")
ng_n = cv2.imread("icon/ng_n.png")
ng_n = cv2.imread("icon/ng_n.png")
ng = cv2.imread("icon/ng.png")
...
# show icon
def draw_icon(frame, icon):
    #h, w = (image.shape[:2])
    #icon = cv2.resize(icon, (150, 247))
    x, y = 485, 40
    frame[y:y+198, x:x+120] = icon
    return frame
...
# Initial RTSP stream
RTSP = "rtsp://10.21.1.4:8554/preview"
try:
...
# tracker + smoother
tracker = SimpleTracker(max_age=40)
...
while True:
    ...
    if active_pids == []:
        ...
        target_person = None
        ready = False
        reminder = "Align your body with the human outline"
    else:
    ...
    # Vote
    if not ready: 
        # no person, back, close, not in human shape -> reset
        agg.reset() 
        #cv2.putText(output, reminder, (20, 620), FONT, 1, COLOR_REMINDER, 2, cv2.LINE_AA) 

        #  Current
        latest_result = reminder
        color_result = COLOR_REMINDER
        human_color = COLOR_NG
        flag_checking= True
        
        # Remind if person stands in an unavailble area too long
        if reminder_frames % (FPS_EST * VOICE_DELAY_SEC) == 0:
        ...
        human_color = color_result

    # guidance
    if flag_checking:
        cv2.rectangle(output, (0, 0), (640, 35), COLOR_GROUND, -1)
        cv2.putText(output, "Please move until the outline turns blue", (30, 30), FONT, 0.8, COLOR_Pending, 2, cv2.LINE_AA)
    else:
        draw_icon(output, icon)

    # change color of human shape
    output=draw_human_shape(output, human_color)

...
class MinuteAggregator:
    def __init__(
        ...
        self.state_counter = defaultdict(Counter)             # (pid, minute): Counter(state)
        self.last_seen: Dict[Tuple[int, datetime], str] = {}  # ((pid, minute), last state)
        self.last_minute_of_person: Dict[int, datetime] = {}  # (pid, last minute)
        self.latest_hits: Dict[int, int] = {}                 # (pid, last frs)

        # Write alarm
        self.alert = alert
        self.ng_streak: Dict[int, int] = {}            # (pid, streak - number of NG case )
        self.ng_threshold_minutes = ng_threshold_minutes                  # K continuous minutes
    ...
    def ingest_rows(self, rows):
        ...
        for r in rows:
            ...
            # 2) If start a new minute -> flush the old minute.
            prev_minute = self.last_minute_of_person.get(pid) # return previous minute of pid
            if prev_minute is not None and prev_minute != minute: 
                row = self._flush(pid, prev_minute)
                if row and str(row["state"]).startswith("NG"):
                    flushed_rows.append(row)

            # 3) Count state: {(pid, minute): Counter({'OK': 2, 'NG_H': 1, 'NG_R': 1, 'NG_HR': 1})
            self.state_counter[(pid, minute)][state] += 1
            self.last_seen[(pid, minute)] = state
            self.last_minute_of_person[pid] = minute
            self.latest_hits[pid] = hits

        return flushed_rows

    # flush the previous minute.
    def _flush(self, pid: int, minute: datetime):
        # take the values and delete this values
        cnt = self.state_counter.pop((pid, minute), Counter())
        if not cnt:
            return
        ...
        if len(top_states) == 1:
            ...
            if self.tie_break == "top":
                ...
                maj_state = self.last_seen.get((pid, minute), top_states[-1])
        self.last_seen.pop((pid, minute), None) # release the variable

        # Take the information of trackID for minute log
        ...
        # save to file minute log
        if self.save_log:
            csv_path = self._build_path_for_minute(minute)
            self._write_row(csv_path, row)

        # Update streak by minute
        ...
        handlers = {
            ...
            "terminal": lambda: threading.Thread(
                ...
                kwargs=dict(
                    message=f"PersonID{pid} violated the rules for {streak} continuous minutes: {maj_state}.",
                    duration_ms=5000
                ),
                daemon=True
            ).start(),
        ...
        if streak >= self.ng_threshold_minutes:
    ...
    def _build_path_for_minute(self, minute: datetime, **kwargs) -> Path:
        date_str = minute.date().isoformat()  # YYYY-MM-DD
        return self.report_dir / f"minute_log_{date_str}.csv"
    ...
    # Flush the remaining minutes when pausing the program
    def shutdown(self):
        for (pid, minute) in list(self.state_counter.keys()):
            self._flush(pid, minute)
